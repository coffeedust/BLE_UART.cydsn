...
void App_UART_BLE_Rx(uint8 *data, uint16 len);
void App_UART_BLE_Tx(uint8 *data, uint16 len);
    ...
    switch(event)
    {
        ...
        ***********************************************************/
        ...
        case CY_BLE_EVT_GATTS_WRITE_REQ:
        {
            ...
            cy_en_ble_gatt_err_code_t gattErr = CY_BLE_GATT_ERR_INVALID_HANDLE;
            
            if((writeParam->handleValPair.attrHandle == CY_BLE_UART_SERVICE_RX_CHAR_HANDLE) || (writeParam->handleValPair.attrHandle == CY_BLE_UART_SERVICE_TX_CLIENT_CHARACTERISTIC_CONFIGURATION_DESC_HANDLE)) {
                gattErr = Cy_BLE_GATTS_WriteAttributeValuePeer(&appConnHandle, &writeParam->handleValPair);
                
                if((CY_BLE_GATT_ERR_NONE == gattErr) && (event == CY_BLE_EVT_GATTS_WRITE_REQ)) {
                    Cy_BLE_GATTS_WriteRsp(appConnHandle);
                }
                if(writeParam->handleValPair.attrHandle == CY_BLE_UART_SERVICE_RX_CHAR_HANDLE) {
                    App_UART_BLE_Rx(writeParam->handleValPair.value.val, writeParam->handleValPair.value.len);
                    App_UART_BLE_Tx((unsigned char *) "test message", 12);
                }
            }
            if((gattErr) == (event == CY_BLE_EVT_GATTS_WRITE_REQ)) {
                cy_stc_ble_gatt_err_param_t err_param = {
                    .errInfo.opCode = CY_BLE_GATT_WRITE_REQ,
                    .errInfo.attrHandle = writeParam->handleValPair.attrHandle,
